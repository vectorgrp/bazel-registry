...
def _import_evs_impl(ctx):
    ...
    return _cli_cmd(
        ctx,
        arxml_files,
        'import evs{snp} -b "{bsw}" -p "{dvjson}" "{evs}"',
        snp = snp,
...
def _derive_ecuc_impl(ctx):
    return _cli_cmd(
        ctx,
        ctx.files.extract,
        'project derive-ecuc -b "{bsw}" -p "{dvjson}" --force "{extract}"',
...
def _import_diag_modules_impl(ctx):
    ...
    return _cli_cmd(
        ctx,
        ctx.files.modules,
        'import diagnostic-modules -b "{bsw}" -p "{dvjson}" --force {files}',
        files = files,
    )
...
def _update_project_impl(ctx):
    return _cli_cmd(
        ctx,
        [],
        'project update -b "{bsw}" -p "{dvjson}"{switches}',
    ...
    ctx.actions.run_shell(
        ...
        command = "{unpack} && {command}".format(
            ...
            command = _format_command(
                ctx,
                out.dirname,
                '"{dvcfg}" export run -b "{bsw}" -p "{dvjson}" -o "{out}" -e "{exporter}"{binding_time}{split_pre}{split_post}{args}',
                out = out.path,
    ...
    ctx.actions.run_shell(
        ...
        command = "{unpack} && {command}".format(
            ...
            command = _format_command(
                ctx,
                out.dirname,
                '"{dvcfg}" export run -b "{bsw}" -p "{dvjson}" -o "{out}" -e "{exporters}"{binding_time}{split_pre}{split_post}{args}',
                out = out.path,
    ...
    ctx.actions.run_shell(
        ...
        command = "{unpack} && {command}".format(
            ...
            command = _format_command(
                ctx,
                folder,
                '"{dvcfg}" project generate -b "{bsw_pkg}" -p "{project}"{modules}{excluded}{ext}{type}{keep_tmp}{clean}{no_save}',
