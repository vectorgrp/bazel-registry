...
def _generate_foundation_layer_impl(bsw_pkg = None, output = None, **kwargs):
    if not output:
        fail("The foundation layer output target must be specified.")
...
generate_foundation_layer = macro(
    ...
    attrs = {
        ...
        "output": attr.label(doc = "The destination folder for generated sources.", allow_single_file = True),
    },
