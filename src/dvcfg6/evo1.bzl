load("@rules_cfg6//:rules.bzl", "default_http_archive_attrs", "download_and_extract", "absolute_path")

def _evo1_archive_impl(repository_ctx):
    folder = repository_ctx.getenv("LOCAL_DVCFG6_EVO1", "_")
    if folder == "_":
        ...
        folder = repository_ctx.path(folder)
    else:
        folder = absolute_path(repository_ctx, folder)
    repository_ctx.delete("_download_")
    repository_ctx.file("BUILD.bazel")
    ext = ".exe" if repository_ctx.os.name.startswith("windows") else ""
    repository_ctx.file("defs.bzl", 'evo1_gui = "{}"'.format(str(folder.get_child("dvcfgui-b", "dvcfgui-b" + ext)).replace("\\", "/")))
